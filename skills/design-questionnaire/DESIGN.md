...
- 浅分支内联限 1 层;题量每波上限 10 题(2026-08-08 收紧,原 10–15),超量拆子波
...
## 格式反馈同步(2026-08-08,grill-Q 压测补充声明用户裁决)

> 来源:grill-harness-file-mgmt-w01 压测补充声明「单波次上限调整为10个,直接问答上限调整为不多于3个」+ 后续确认「四份统一 ≤3」。OD-8 重访触发①:四副本同步。

| 项 | 原 | 新 | 落点 |
|---|---|---|---|
| 题量上限 | 每波 10–15 | 上限 10 | 四份 QUESTIONNAIRE-FORMAT 规则 8 |
| 小波阈值 | design/grill/retro ≤2;action-Q ≤4(有意分叉) | 四份统一 ≤3 | 四份 SKILL.md 第 2 步 + PROCESSING-RULES 提问波 |

- action-Q 的 ≤4 → ≤3 为**有意分叉修订**(OD-11 原分叉声明更新),不撤销 confirm-list 语义分叉。
- 双向门;题量/阈值参数可回退。
...
## SKILL.md 分层迁移评估(2026-08-19,grill-Q skill-family W01 Q8-A / ADR-0023)

> 样板迁移结论:design-Q 第 92 行 dogfood 纯过程叙述收拢为一行 + 指向本节「dogfood 修订」(SKILL.md 规则本体不动)。**全量分层迁移评估为低收益、缓行**——
> - 扫描实测:9 skill 的日期注记绝大多数是**规则出处与原值**(如「上限 10,原 10–15」「≤3,原 ≤2」),属规则语义不可分割部分,删则规则变无源教条(ADR-0023 后果(−)已预判);真正可剥离的纯过程叙述(裁决史 / dogfood 修订史)占比 < 5%。
> - long-running 344 行主体是检查清单 / 规范本体(必要复杂度),非教训堆积;「不再使用 init.sh」类注记是防 AI 复活旧行为的规则反例,保留。
> - **ADR-0023 的主要价值 = 升格机制(教训重复 ≥2 次才常驻)+ 新增教训归位纪律(新教训先落 DESIGN.md 历史层)**,而非存量一次性大迁移。存量分层随各 skill 自然大修时顺带做(顺手收敛,同铁律 7 / Q7-C 策略)。
> - 后续触发:某 skill 因其他原因大修时,顺带核对其日期注记是否可收拢;全量大扫除不立项。
\n
## grill-design-q 压测修订(2026-08-20,grill-Q 三透镜压测 W01)

> 来源:[grill-design-q-w01](../../harness/questionnaires/archive/_misc/grill-design-q-w01.md)(软件工程 / 第一性原理 / 项目哲学三透镜,10 题;入口校准闸门已过)。9 项修订全执行(⚠ 两项草案另行确认后落地);残余张力与观察项落 [OD-27](../../docs/OPEN-DECISIONS.md) / [OD-28](../../docs/OPEN-DECISIONS.md)。

| # | 决策 | 结论 | 出处 |
|---|---|---|---|
| D31 | W00 价值定位 | 全量维持,防漏优先于防重复问(否决 Q6-A 数据驱动收窄);**护栏观测与机制审判分离**——Q4 质量信号(全采纳率,防默认效应)落地,Q6 统计(取消率/转 W01 数,验证 W00 形态)不做,防混用 | Q6-C + Q4-A |
| D32 | 入口校准闸门 | 每层生成 W00 前出示「项目理解摘要 + 本层设计焦点」AskUserQuestion 确认;轻量模式豁免;与 grill-Q 入口闸门对称(CONTEXT「AI 同源出题限定」体系内闸门扩至生成式侧) | Q3-A |
| D33 | 收尾面板化 | 四项衔接(dogfood/压测/多线程必停/long-running)合并为单次 AskUserQuestion 多问题面板;多线程必停居首问,裁决语义不变(2026-08-16 必停 + 2026-07-24/28 提议机制全保留) | Q7-A |
| D34 | 处理报告全文归档 | 归档时全文追加问卷尾部(每题去向/异常/逃生舱/下一波候选/覆盖度),摘要节保留速览——「原始信息不丢失」覆盖解析产物 | Q9-A |
| D35 | W00 推理句分级 | 高 stakes/单向门要点默认倾向后附一句推理(不限字数,✍️ 修正原 ≤15 字限制);低 stakes 从简——L2 留痕按哲学「度」分级 | Q8-B + ✍️ |
| D36 | delegate 白名单接口 | 触发三条件与(feature 模式 + delegation.md 存在且开关开 + **人当次会话明示**,防过时白名单静默生效);白名单内不出独立题、合并**单题「白名单包确认」**(整包确认/逐条挑出/整包否决,保人可见性只降交互粒度);留痕双落(处理报告清单 + delegation-log 逐例);残余张力(init/无白名单 vs §4.5)落 OD-27 | Q5-C + ⚠ 三点细则(自动决策进 log / 触发当次明示 / 包确认题) |
| D37 | 形态×协议交叉表 | STAGE-SKELETONS 补「形态 × 协议」交叉表(轻量/单文件 × 闸门/自检/回退/插层;表内容经 ⚠ 确认后落地);「修订自检清单」不建,进 OD-28 观察(对齐 ADR-0023 升格机制) | Q10-A |
