...
## AI 黑盒(v5 第四学科视角立论锚点 · grill-with-docs 深钻结晶,2026-08-10)

> 安全科学作第四学科视角的立论锚点;v5 已挂接([philosophy_v5 §八](methodology/philosophy_v5.md#八安全科学视角去-ai-黑盒))。深钻来源:grill-with-docs「去 AI 黑盒」单点深钻(2026-08-10,6 点结晶)。

- **AI 黑盒** = AI 不透明的三层次合一:① **决策过程不透明**(推理链不可见);② **决策依据不可追溯**(为什么选 X 不选 Y 无留痕);③ **产出正确性不可独立验证**(功能正确性有客观裁决源但常由 AI 自写——嵌套黑盒;判断类产出只能人审,详见 [philosophy_v5 §8.1](methodology/philosophy_v5.md))。
...
- **对策方向**:统合已有可审计装置(delegation-log / ADR / 归档问卷 / Code Review 人审 / dogfood 留痕 / 脱敏门 / harness-check / retro 复盘 / long-running 留痕)为「对抗黑盒」立论——黑盒被制衡而非完全打开(L1 推理链无对策,留痕 = AI 自陈理由);形式化 V&V 缺口(结果可独立验证)留 [OD-19](OPEN-DECISIONS.md)。
