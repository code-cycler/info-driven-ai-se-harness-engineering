...
- 每个 en 镜像文件头部必带三字段标记(YAML frontmatter;已有 frontmatter 的文件如 SKILL.md 在既有 frontmatter 内追加):
  ...
  - `zh-hash: <源指纹>`(见 §2.1)
- 载体已终裁:**维持 frontmatter**(2026-08-23 grill-Q Q10:字段机器可查、语义标准,GitHub 顶部表格是可忍的诚实元数据;换 HTML 注释需改解析器 + 全部 en 文件,纯为视觉不值)。
...
- **mermaid 图内文本标签(label)译英**——语法结构与节点 ID 不动,仅译引号内文本与边标签;渲染验证人审兜底(图坏则回退原文重译)。〔2026-08-23 dogfood 修订:原「mermaid 原样不译」的两条理由复核——「防图漂移」不成立(图在文件内,文件级 zh-hash 已覆盖:中文图变 → hash 变 → 报过期 → 回译同步图),仅语法风险真实且可控;README 协作图是承重件,英文读者在图处断线的代价大于译图成本。触发 = 人审质询,用户裁决「修订并译图」〕**标签内引号规则**(2026-08-25 人审实锤:philosophy_v7 §1.1 图用了转义直引号 `\"…\"`,mermaid 解析失败——zh 原文「」定界符天然无冲突,英文译者须改用排版引号 “…” 或单引号,**禁止 `\"` 转义**;同类 = 标签内禁裸双引号)
