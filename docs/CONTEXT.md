...
## 推进受挫谱系(2026-09-09,grill-with-docs 深钻结晶)

> 来源:哲学 v7 挑战报告(游离)挑战一「返工无操作化定义 → 核心因果链免疫检验」深钻,用户四问裁决:谱系化+锚定子类 / 三字段事件制 / 补测式死亡快照 / 完整落盘。挑战一据此从「认识论级致命」降为「高——已修复待实测」(残留:无对照/基线的个人开发者结构性限制,只能自身时间序列积累重访信号)。

- **推进受挫** = 症状层顶层概念(作者初始意图补述,落笔为「返工」的差异属 (a) 类盲区活例):机制层导致的推进障碍,三结局谱系——**搁置/放弃**(项目级死亡)/ **返工**(项目继续推进时的可观测症状)。过新词三条件:项目管理 abandonment/stalled 无三结局统一谱系对应,作者特有结构。
- **返工** = 推进受挫的**可测量锚定子类**,继续承担三层模型症状层职能(顶层概念不替换它)。操作化定义(**三字段事件制**):**返工事件** = 曾被认为「完成/可用」的产出,核心假设被推翻,须重新投入才能恢复推进——不问归因,归因靠字段。retro 采集三字段:① **深度**:**重构型**(推翻契约层假设——接口/架构/数据模型/选型,改动半径越过模块边界)/ **缝补型**(契约不动,局部修补实现);② **粗归因**四选一:信息断层/能力边界/需求漂移/集成意外(不求精确,求 MC-01「反例连续出现」可识别);③ 耗时估计(可选,不强制)。状态:规范已定义,未实测(首次 retro 采集后升「本仓库已实测」)。
- **受挫死 / 自然死** = 搁置/放弃结局的死亡状态二分:**受挫死** = 死时有未解决返工积压(卡住的 feature_list 项/修不完的 bug 链/TODO 停滞);**自然死** = 干净收官或干净转移(个人项目正常代谢,与因果链无关)。判定材料全部为仓库客观状态,支持**补测式死亡快照**(死后任意时刻从 feature_list/TODO/git 补判,不依赖临终配合;retro-Q 触发场景含「项目搁置/放弃时(建议非强制)」);不可重建的仅自由文本死因(信号量最低,接受丢失)。
...
> **同步指针(2026-08-20,readme-revamp W01 Q2)**:修订本表维度列时,README「各 skill 提问/确认维度速查」表必同步(README 为导览副本,权威 = 本表)。
...
> **生存者偏差限定(2026-09-09,grill-with-docs 挑战一深钻)**:搁置/放弃型推进受挫是已知测量盲区——渐进失血式搁置的观测窗口常在数据应被采集前关闭。对策 = 补测式死亡快照(受挫死/自然死从仓库客观状态事后可判,见「推进受挫谱系」节),但快照 compliance 预期低(人因:项目死亡时无人想做 retro),预期归宿 = 盲区永久存在,只标注不假装可测。重访触发 = 首次死亡快照补测真实发生,或 ≥2 个项目死亡仍零补测时正式降格为「承认盲区零装置」。
