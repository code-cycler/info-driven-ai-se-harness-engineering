...
> **学科视角**(v7):§一挂人因工程、§二挂软件工程、§三挂运筹学、§四挂安全科学;本版继承 v6 的治理进化路线,但不把所有映射学科升级为正文立论学科。哲学主张骨架用成熟学科语言,方法论文件保留操作语言。**诚实标注**:学科挂接是阐述性借用(用学科语言为主张做结构化解释),非经学科方法(人因实验 / 运筹建模 / 软工实证 / 安全分析)验证——方法论核心主张的证据必须由验证卡、dogfood、独立人审与 retro 共同提供,不靠学科权威或 AI 自评。
...
- **人与 AI 之间的断层**(vibe coding 失败场景的抽象):你知道的背景没传给 AI,AI 在信息真空中自作主张。**解法:问答对齐**——用结构化追问(Grill 家族,见[方法论文件第五章](methodology_v5.md#四grill-决策方法论))把背景逐步对齐给 AI,这是[方法论文件 §2.1](methodology_v5.md#11-第一支柱以信息为核心)机制层的直接对策。
...
决策串行不仅因为"等人",还因为"每问等一轮 LLM 输出"。把一问一答的 Grill 改为**多波次批量问卷**(见[方法论文件 §5.2-5.3](methodology_v5.md#42-演进说明从一问一答到批量问卷)),**减少 LLM 串行往返**——一次出题(每波上限 10)、离线作答,把等待成本压到最低。注意:判断本身仍逐题串行,并行的是「出题与作答准备」而非「决策」本身。
...
**最小验证卡 `MC-01`**:本节的主张是「信息断层会增加 AI 自作主张与返工风险」;适用范围是需求、设计、工具或外部依赖信息不完整的任务;当前状态为**目标治理能力 / 未验证**。最低证据是保留原始事实或可复现命令 / 输出,区分事实、假定与未验证项,并在采用项目的 retro 中对照返工定义、基线与观察窗口。可能反例包括返工由需求漂移、AI 能力边界或集成意外主导;若反例连续出现或代理指标与主张边界不一致,重访本主张。完整卡片字段见 [方法论治理最小切片](../../harness/design/methodology-governance/LLD.md)。
...
> **软件工程视角**(v4):本章从软件工程视角看分工——工程纪律(设计先行 / TDD / 测试 / ADR / 复盘)是 AI 产出的**护栏**(方法论文件 [§2.2](methodology_v5.md#12-第二支柱驾驭工程--ai--软件工程)机制层);分工表的本质是「决策权在人、执行权在 AI」,工程纪律让这种分工可审计、可回退。
...
两族是同一判断成本优化系统的两个层位,按依赖链深度、即时反馈需求、可逆性 / 风险和能否离线作答分流,经「深水区转单点深钻 / 结晶成工件复压」双向交接(见[方法论文件 §4.3.1](methodology_v5.md#331-skill-生态位路由与跨-skill-最小契约v7-w02)与 [§5.2](methodology_v5.md#42-演进说明从一问一答到批量问卷))。
...
九个 skill 的共同契约只约束五件事:输入 / 输出、证据状态、偏差记录、验证强度、交接触发。它们解决的是生态位可见性和责任可追溯,不是新增第十个总控权威。唯一权威路由表与覆盖矩阵由 [方法论文件 §4.3.1](methodology_v5.md#331-skill-生态位路由与跨-skill-最小契约v7-w02) 和 [治理最小切片](../../harness/design/methodology-governance/LLD.md) 承载。
