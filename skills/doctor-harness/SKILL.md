...
- **判定**:本次是要① 组织规则(落到 HARNESS-RULES.md)② 迁移执行(重组目录)③ 校验(跑脚本)④ 留痕(记录变更)⑤ 层级改造(旧三件 → LN 制迁移,按 HARNESS-RULES 第七/八节)⑥ 存量规范化(无 harness 文件项目 → 标准结构,按第八节五步流程)⑦ **治理历史布局**(历史载体 CHANGELOG/FORK-NOTES/DOGFOOD-LOG/STATUS-LOG 的建立与迁移、历史注记从规则文件剥离、增量记录五类触发,按第九节/ADR-0024)——可单独或组合。
...
- 读 [HARNESS-RULES.md](./HARNESS-RULES.md)(唯一权威源),确认是否覆盖当前场景:
  ...
  - **治理历史布局(第九节,ADR-0024)**:载体命名与粒度(skill CHANGELOG 仅项目侧/FORK-NOTES 双侧一致/DOGFOOD-LOG 仅全局侧/STATUS-LOG);历史层单侧存在规则(sync-check 类规则);索引指针要求;增量记录五类触发 + 节标题改名时的锚点更新义务。
