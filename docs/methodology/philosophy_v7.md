...
> **修订记录**(仅记录已提交或已发布版本,一行一级;工作树中的执行记录见 TODO / 问卷处理摘要):2026-08-14 v7 在 v6 基础上完成连续章节、独立阅读入口、历史编号兼容与双文件交叉治理契约;**2026-08-19 版本内修订:grill skill 退役(归 waste/),家族由 9 收为 8,§3.1 单点深钻族与 §五「九个 skill」同步更新(内容修订非结构变化,按 canonical 升版判据不升版)**;**2026-08-19 版本内修订:§3.1 最小路由表加「认知状态」行(grill 边界深钻 + grill-boundary-canonical-w01 复压,内容修订非结构变化,不升版)**。
> 三块关系:方法论 = [methodology_v5.md](methodology_v5.md)(怎么做,自包含);哲学 = 本文件 v7(为什么);实操 = [practical_v1.md](practical_v1.md)(怎么用,非 canonical 轻量修订)。
> **章节编号(v7)**:哲学正文使用独立连续编号「§一至§五」;旧 v3 编号映射为「旧 §一→新 §一、旧 §六→新 §二、旧 §七→新 §三、旧 §八→新 §四、元原则→新 §五」。历史文档保持原编号,兼容策略见 [ADR-0017](../../harness/adr/0017-philosophy-section-compatibility.md)。正文中的「方法论 §x」引用仍指 [methodology_v5.md](methodology_v5.md)。
> **学科视角**(v7):§一挂人因工程、§二挂软件工程、§三挂运筹学、§四挂安全科学;本版继承 v6 的治理进化路线,但不把所有映射学科升级为正文立论学科。哲学主张骨架用成熟学科语言,方法论文件保留操作语言。**诚实标注**:学科挂接是阐述性借用(用学科语言为主张做结构化解释),非经学科方法(人因实验 / 运筹建模 / 软工实证 / 安全分析)验证——方法论核心主张的证据必须由验证卡、dogfood、独立人审与 retro 共同提供,不靠学科权威或 AI 自评(**独立人审 = 体系外第二人;当前 N=1 单主体不可满足,以外部 issue / PR / 采用反馈为代理证据源**,见 [CONTEXT 证据状态节](../CONTEXT.md))。
...
- **人与 AI 之间的断层**(vibe coding 失败场景的抽象):你知道的背景没传给 AI,AI 在信息真空中自作主张。**解法:问答对齐**——用结构化追问(Grill 家族,见[方法论文件 §四](methodology_v5.md#四grill-决策方法论))把背景逐步对齐给 AI,这是[方法论文件 §2.1](methodology_v5.md#11-第一支柱以信息为核心)机制层的直接对策。
...
决策串行不仅因为"等人",还因为"每问等一轮 LLM 输出"。把一问一答的 Grill 改为**多波次批量问卷**(见[方法论文件 §4.2–4.3](methodology_v5.md#42-演进说明从一问一答到批量问卷)),**减少 LLM 串行往返**——一次出题(每波上限 10)、离线作答,把等待成本压到最低。注意:判断本身仍逐题串行,并行的是「出题与作答准备」而非「决策」本身。
...
**最小验证卡 `MC-01`**:本节的主张是「信息断层会增加 AI 自作主张与返工风险」;适用范围是需求、设计、工具或外部依赖信息不完整的任务;当前状态为**目标治理能力 / 未验证**。最低证据是保留原始事实或可复现命令 / 输出,区分事实、假定与未验证项,并在采用项目的 retro 中对照返工定义、基线与观察窗口。可能反例包括返工由需求漂移、AI 能力边界或集成意外主导;若反例连续出现或代理指标与主张边界不一致,重访本主张。**模型行为假设**(2026-08-18,first-principles W01 Q2):本主张依赖「AI 在背景缺失时不主动声明不确定性、以最合理猜测补全」——这是当前模型代际的行为观察,非结构性真理;若模型默认输出置信度 / 主动说「我不知道」,本卡前提失效,须重访。完整卡片字段见 [方法论治理最小切片](../../harness/design/methodology-governance/LLD.md)。
...
> **软件工程视角**(v4):本章从软件工程视角看分工——工程纪律(设计先行 / TDD / 测试 / ADR / 复盘)是 AI 产出的**护栏**(方法论文件 [§1.2](methodology_v5.md#12-第二支柱驾驭工程--ai--软件工程)机制层);分工表的本质是「决策权在人、执行权在 AI」,工程纪律让这种分工可审计、可回退。
...
§1.2 的**人与人之间断层**(需求传递失真、等 PM/QA)在这一节落地为解法:AI 替代传统角色中的**结构化执行与等待链**,减少"等人"与传递失真。

在传统团队中,以下角色需要不同的人担任,产生沟通延迟和上下文切换成本。在小型团队 + AI 模式下,这些角色中的结构化任务由 AI 替代或辅助,**压缩等待时间**;目标、风险、验收、责任和签字权仍不替代:


| 传统角色          | 传统时间成本                                               | AI 模式(仅执行 / 等待链)                    |
| ------------------- | ------------------------------------------------------------ | -------------------------------------------- |
...
**关键变化**:不是 AI 接管了角色责任,而是 AI 消除了其中可结构化执行的"等人"时间。角色名不能推出责任替代;凡涉及目标、风险、验收、接口或不可逆状态的判断,仍由人选择和确认。
...
方法论把「人的判断精力」当作有限资源来配置——这是 **80/20 判断成本原则**的注脚(经验启发式,类 Pareto;**非定理、无量化模型,也不是时间配额**;原初设计 2026-07-31 作者补述、此前未落盘,属 (a) 类盲区「知道但没写」的活例):

- **批量问卷族**(design-Q / grill-Q / retro-Q / action-Q)=可预知、问题空间已知、可离线回答的基础问题层。
- **单点深钻族**(grill-with-docs,含绑库 + 通用双模式——原 grill 已于 2026-08-19 退役并入通用模式,见 [OD-12](../OPEN-DECISIONS.md))=依赖链深、决策未成形、需即时反馈或必须绑定现实证据的关键问题层。

两族是同一判断成本优化系统的两个层位,按依赖链深度、即时反馈需求、可逆性 / 风险和能否离线作答分流,经「深水区转单点深钻 / 结晶成工件复压」双向交接(见[方法论文件 §3.3.1](methodology_v5.md#331-skill-生态位路由与跨-skill-最小契约v7-w02)与 [§4.2](methodology_v5.md#42-演进说明从一问一答到批量问卷))。

**最小路由表**:

| 判据 | 批量问卷族 | 单点深钻族 |
|---|---|---|
| 认知状态(答案材料的位置,2026-08-19,复压 grill-boundary-canonical-w01) | ① 知道·可离线答——答案材料已在人脑,只是没写((a) 类) | ② 知道方向·决策未成形——答案需依赖链逐轮推演生成、需即时反馈;③ 不知道自己不知道((b) 类隐含假设)——两族共管:批量族对抗维度(D1/D5/D7)逼出,深钻族以具体场景测边界 |
| 问题形态 | 可预知、可拆成多题、可离线 | 下一步取决于上一轮回答,或问题尚未成形 |
| 现实绑定 | 不需要即时读取代码 / 外部系统 | 必须与代码、文档、工具或外部依赖交叉核验 |
| 风险与门型 | 普通可逆、影响可控 | 单向门、高风险、影响范围不明 |
| 反馈要求 | 可批量解析后再处理 | 需要即时反馈才能避免错误分支 |
| 交接 | 深水区转 `grill-with-docs`;稳定结论回工件 | 结晶为工件后回 `grill-questionnaire` 复压 |

这张表是路由启发式,不是新的阶段或时间预算;真实案例中的误触发和交接失败才是重命名或增设治理入口的依据。
...
AI 黑盒 = AI 不透明的**三个相互独立、可叠加的维度**;它们不是同一层级的同义改写,一个维度的证据不能替代另外两个维度:

- **L1 决策过程不透明**:你看到 AI 的输出,但不知道它**怎么推到的**(推理链不可见)。
- **L2 决策依据不可追溯**:AI 选了 X 不选 Y,但**事后无留痕可查**(为什么这么决策查不到)。
- **L3 产出正确性不可独立验证**:功能正确性有客观裁决源(oracle:编译 / 测试 / E2E),但裁决源常由 AI 自写——AI 写测试测 AI 代码,独立验证的独立性打折(**嵌套黑盒**,[OD-13](../OPEN-DECISIONS.md) 实证「自证清白」);判断类产出(设计好坏 / 需求符合)无 oracle,只能人审。L3 的最低 assurance 必须按风险匹配证据强度,不能把轻量测试绿扩大成整体结论。
...
| 装置 | 规范已定义 | 执行体可调用 | 本仓库已实测 | 下游项目需接入 / 补证 |
|---|---|---|---|---|
| ADR / CONTEXT / OPEN-DECISIONS | 已定义记录、术语和待决策边界 | Git + Markdown 可调用 | W01/W02 处理已形成 ADR / OD / TODO 证据 | 按项目维护唯一权威与重访条件 |
| 归档问卷 | 已定义只移不删与处理记录 | `harness/questionnaires/` 可调用 | v7 W01/W02 已归档并通过布局校验 | 下游保留回答、处理和修订授权链 |
| `harness-check` / 脱敏门 | 已定义发布前布局与脱敏门 | `scripts/` 可调用 | 本轮均为 0 命中 / 0 违规 | 下游按发布门实际运行并保存输出 |
| Code Review 人审 | 已定义最终签字权在人 | 由宿主工具 / 流程接入 | 本仓库未以 active `pr-review` skill 证明 | 下游接入人审记录,不能以 AI approve 替代 |
| dogfood / retro / delegation-log / long-running | 已定义目标流程与记录形态 | 对应 skill 可调用 | 本仓库尚无完整项目级 pilot 闭环 | 下游需按风险接入并补真实执行证据 |

这张表只证明状态分层,不证明下游效果。最小证据必须能回到原始来源、可复现命令 / 输出、人审 / 试玩记录或处理报告;没有证据的单元保持“未验证”。
...
**L3 风险分层原则**:凡 AI 自写 oracle、用户可玩产物或单向门操作,必须有人的独立审查、试玩或确认;普通可逆代码可用客观测试并标明范围;高风险或出现反例信号时升级覆盖矩阵、Assurance Case 或更强独立验证。具体采样频率、风险阈值与项目 DoD 放在实操层定义,本节只规定最低边界。测试通过只证明测试范围内的断言,不自动证明整体结论。
...
「按项目、场景和工具调整」不意味着可以静默取消最低安全边界。以下四项不可裁剪,其余流程、记录粒度和工具选择才按风险裁剪(详见 [ADR-0019](../../harness/adr/0019-methodology-nonnegotiable-guardrails.md)):

1. 判断性决策与最终确认由人承担,AI 不替人决定目标、风险取舍、验收或签字。
2. 单向门先停并留痕,删除、发布、付费、外传、外泄及等价不可逆风险必须先确认。
3. L3 必须有独立验证,AI 自评不能单独证明测试、试玩或单向门操作通过。
4. 冲突不得静默覆盖,必须保留事实与副作用,标记影响范围并进入恢复或重访路径。

这四条合同把散文主张连接到可验收的过程,也给方法论自身设置与 §4.5 一致的弹性边界:

1. **唯一权威**:每条核心方法主张都应有稳定标识、适用范围、唯一权威文件、违反症状和例外边界。规范优先级只能回答文件冲突时听谁的,不能替代「这条规则由谁负责维护」的唯一权威;跨 skill 传播时还要指明责任 skill。
2. **主张 → 不变量 → 验证卡**:主张先压缩为可检查的方法不变量,再配验证卡。验证卡至少说明反例、代理指标、独立证据和阈值或重访条件;首批卡片见 [方法论治理最小切片](../../harness/design/methodology-governance/LLD.md),哲学不膨胀成规格书。
3. **失败状态与恢复边界**:发现文档、执行与现实冲突,或发现审查缺口时,依次执行「标记影响范围 → 暂停不可逆动作 → 保留事实与副作用 → 选择性恢复或回滚 → 复盘并更新规范」。设计—实现偏差先分类;各 skill 可定义具体状态和动作,但不能用覆盖文件或静默重试掩盖失败。
4. **规范、实现、证据分离**:「应该遵守」不等于「已经执行」,架构或 skill 存在不等于「已经实现」,本仓库的过程检查也不等于下游效率、质量或因果效果的证据。涉及工具、代码、外部依赖或不可逆动作时,证据必须前置;状态应明确区分规范要求、启发式、试点、已验证、未验证和已知缺口,证据范围与状态定义见 [CONTEXT](../CONTEXT.md)。
...
#### 4.6.3 跨 skill 协作的治理边界

八个 skill 的共同契约只约束五件事:输入 / 输出、证据状态、偏差记录、验证强度、交接触发。它们解决的是生态位可见性和责任可追溯,不是新增第九个总控权威。唯一权威路由表与覆盖矩阵由 [方法论文件 §3.3.1](methodology_v5.md#331-skill-生态位路由与跨-skill-最小契约v7-w02) 和 [治理最小切片](../../harness/design/methodology-governance/LLD.md) 承载。(2026-08-19:grill 退役,家族由 9 收为 8;「不新增总控权威」由「第十个」改为「第九个」)

跨 skill 交接还必须遵守两条边界:

- **实现不能静默改写设计**:局部、不改变目标 / 约束 / 风险 / 接口 / 验收的优化可由实现记录或 retro 回灌;改变其中任一项的偏差必须先暂停不可逆动作,更新 canonical 设计并重新交叉挑战。
- **失败不能只在末端补记**:设计期列关键边界和回滚,压测期挑战反例,实现期用 dogfood / 测试验证代表性场景,retro 把漏项回灌为规则或验证卡;未覆盖边界必须显式承认。

这使方法论吸收同级对标仓库的合同化优点,但不把当前个人项目改造成完整的运行时状态机、Level 1/2/3 实现或强制全量追踪系统。
...
> **当前状态注(v7)**:本文件允许存在已知缺口,但不把规范文字当作实施或效果证据。首批 4 张方法不变量 / 验证卡已经落盘,尚未形成全量验证集,也尚未被真实案例 dogfood 充分验证;本节代理指标仅是反思提示,不是下游效果证明;前三行最小进入/退出模板仍待 dogfood。完整 Level 1/2/3 实现仍是 [OD-20](../OPEN-DECISIONS.md) 的开放问题。
...
> **清单证据分级**(2026-08-18,grill-Q first-principles W02 T2):本表混合**实证条**(有案例锚点,如引擎漂移→OD-8)、**预防条**(标 ★,压测产出的防误读规则,未发生事故)与**经验条**(通用工程经验,无本仓库案例)——三类证据强度递减,均属预防目的而非实证报告。
...
| **设计-实现脱节**        | 精心写设计,实现走了不同的路,设计文档不更新              | 先分类偏差;改变目标 / 约束 / 风险 / 接口 / 验收的偏差暂停不可逆动作,更新 canonical 设计并交叉挑战 |
...
| **证据强度越界** ★      | 测试变绿、文档存在或 AI 自评被扩大为整体结论             | 风险分层 assurance;标明范围、未覆盖边界和升级信号                |
| **skill 交接模糊** ★    | 相近入口误触发,责任和产物在 skill 间漂移                 | 使用唯一路由表与五字段共同契约;真实案例记录交接缺口              |
