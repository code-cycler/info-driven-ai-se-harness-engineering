...
| 绿地项目(有 docs 无代码)D7 误判为无效 | GRILL-SKELETON D7 + SKILL 模式判定补「绿地子模式」:D7 的「现实」转为既有设计文档/ADR/约束(工件内部一致性 + 设计 vs 既有约束),D1–D8 全开。项目B dogfood Q15 实证:绿地模式 D7 抓出 ADR-0001「函数指针表」vs LLD「实例字段」不一致(2026-07-27) |
