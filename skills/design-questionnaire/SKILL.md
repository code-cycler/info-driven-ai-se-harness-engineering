...
- 判断 mode:
  - `init` — 新项目 / 从零开始;
  - `feature` — 已有项目的新功能设计:骨架按 [STAGE-SKELETONS.md](./STAGE-SKELETONS.md) 的 feature 裁剪节调整;探索现有代码库是必须前置步骤。
- 若用户没有给初始描述,先要「一句话念头 + 动机」。这是用户的作业,agent 不代写构想。
- **术语映射(LN 制,2026-08-16 P3)**:本文件历史表述中的 vision / hld / lld 分别对应 **L0-vision(目标层,恒在)/ L1-contract(契约层)/ L2-build(构建层)**;问卷 frontmatter `stage` 取**层名**(如 `L0-vision`、`L1-contract`、`L2-build` 或自声明层名);产物文件按 LN 命名(`L<N>-<功能>.md`,见 STAGE-SKELETONS 总则)。旧三件命名(VISION/HLD/LLD)仅存量豁免(布局侧见 HARNESS-RULES 第七节,迁移映射见其第八节)。
- **层数与形态判定**(AskUserQuestion 与用户确认,取代旧三档坍缩):
  - **初始层数 = 最小 1 层**:L0 必生成;是否加 L1/L2 由 **L0 自检五信号**建议 + 人拍板(小项目/小 feature/小 action 单层交付合法);
  - 中大型默认 L0/L1/L2 三层;多子系统按需 L3+(插层协议可中途增);
  - **形态**:多文件(每层一文件,每层独立 W00)or 单文件多节(小项目,整文件一个 W00)——定模时选定。
- **旧档迁移**:存量 VISION/HLD/LLD 设计套迁 LN 制走 doctor-harness(映射表),不由本 skill 顺手改。
...
- **轻量模式(2026-08-19,grill-Q skill-family W01 Q4-B + Q3/Q4 自定义)**:feature 小、单层交付(L0 恒在)、无外部依赖实测需求时,可对人提议走轻量管道——**AI 提议、人拍板,AI 无自裁权**(AskUserQuestion:「判为轻量设计,是否轻量执行?」)。轻量管道 = 调研只读 CLAUDE.md + 直接涉及的代码区(免全量文档地图)+ L0 单层交付 + 优先小波直问 + 免归档(处理摘要入对话);**先验证再出题(铁律 5)/ 环境现实验证不因轻量而豁免**(涉外部依赖仍须实测)。**初步结论先行**:AI 不追求「万无一失」才出手——先出初步设计草案请人轻验证(人勾/否/补一句),把「AI 全量验证」换成「AI 起草 + 人轻验证」,与 [OD-13](../../docs/OPEN-DECISIONS.md)「人类试玩通过才升级」/ OD-19「AI 自写 oracle 必须人审」同构,补其在轻任务层的缺位。
...
### 5. 循环与终止(LN 制:层闸门,2026-08-16 P3)

- 判断当前**层**是否**再无可盘问的信息**,判据:该层最小必含(STAGE-SKELETONS 第二部分)全覆盖 + 动态盲点清零 + 逃生舱项已全部进 OPEN-DECISIONS。
  - 还有 → wave+1,回第 2 步。
  - 没了 → 出示覆盖清单,然后过**层闸门**:用 AskUserQuestion 单独确认一次「本层还有要补充的吗?」。
    - 有补充 → 生成补充波。
    - 无 → **L0 闸门同时过 L0 自检**(五信号建议 + 人拍板增层与否,单层交付合法);进入下一层(或收尾,若末层完成)。
  - **层重开规则**:已过闸门的层因插层/回退协议(上层契约变更或上游前提失效)需要重开时,**只重过受影响层**(非全链重跑);作废/仍有效二分标注,归档问卷不动,重开事实记处理报告。
