...
> 方法论三块拆分([ADR-0007](../../harness/adr/0007-methodology-three-way-split.md))产物之一:**哲学文件**——「为什么」的阐述(失败模式与信息断层 / 人机分工 / 元原则)。v1 起线(2026-08-04)→ v4 学科化(2026-08-05)→ **v5 加安全科学第五学科视角**(2026-08-10:去 AI 黑盒立论锚点,经 grill-Q discipline-mapping + grill-with-docs 深钻 + design-Q + grill-Q 设计压测)。
...
> **安全科学视角**(v5 新增):本章从安全科学(safety science)视角看 AI 的不透明——AI 黑盒是与「AI 决策错」(第一支柱)正交的独立风险维度。安全科学作统称,含**可靠性工程**(Reason 潜伏条件 / HRO 失败预念)+ **韧性工程**(Hollnagel Safety-II,WAI/WAD)的交叉,三分详见 [CONTEXT 学科地图](../CONTEXT.md)。
...
- **产出正确性不可独立验证**:你**无法脱离 AI 自评**来确认它对(AI 说通过了,你没法独立复核)。
...
两者正交——**信息给够仍可能黑盒**(过程不可见),**过程留痕仍可能决策错**(信息缺失)。正交意味着去黑盒够格作独立的第五学科视角锚点,而非第一支柱的附庸。
...
- **失控放大**(安全科学,STAMP):黑盒 = 安全控制过程模型不可见 → 人(控制器)无法判断 AI 是否偏离控制约束 → 局部正确决策在系统层 dysfunctional 交互致灾。
- **信任劫持**(人因工程):黑盒 → 人只能信 AI 自评 → 认知惰性逐渐放弃审查 → 退化为「AI 说对就对」的盲信。

### 8.4 对策:统合已有可审计装置

去黑盒的对策**不是新增机制**,而是把项目已有的可审计装置统合为「对抗黑盒」的立论:

- **delegation-log**(delegate 决策留痕)+ **ADR / CONTEXT / OPEN-DECISIONS**(决策记录)+ **归档问卷只移不删**(过程可追溯)+ **Code Review 人审**(pr-review「AI 永不 approve」)+ **dogfood 留痕**(过程记录)+ **脱敏门 / harness-check**(发布前校验)。

这套装置让 AI 决策「**可审计、可追溯、可回退**」——黑盒被打开。
...
- **WAI 定底线**:关键 / 单向门 / 安全类决策**强制留痕可查**(ADR / delegation-log / 脱敏门);
- **WAD 留空间**:纯执行 / 可逆决策可**批量或事后留痕**(delegation-log 追加式 + 白名单分级)。

这条弹性边界与 Safety-II 的洞见一致:检查清单(WAI)必须**弥合 WAD 缝隙,而非压垮 AI 的适应能力**——系统可靠恰恰因为执行者在 WAD 中灵活调整,去黑盒不能牺牲这份韧性。
