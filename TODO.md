...
### 🟡 哲学 v7 挑战一修复包(推进受挫谱系,grill-with-docs 深钻,2026-09-09)

> 来源:用户发起「挑战本项目的哲学」→ 游离挑战报告(八条,项目根 `philosophy-v7-challenge-report_2026-09-08.md`)→ 挑战一深钻四问裁决:谱系化+锚定子类 / 三字段事件制 / 补测式死亡快照 / 完整落盘。CONTEXT 术语节 + 生存者偏差注 + retro-Q 触发场景已随本深钻落盘;下述 canonical 修订**未执行**(哲学是 canonical 成员,需 canonical 审查:grill-Q 压测或人逐行审 + 用户批准,见 CONTEXT「canonical 审查」条)。

- [ ] **哲学 L83 代理指标段升级**(canonical 修订候选):「返工率/返工工时占比」泛称 → 三字段采集格式(深度[重构型/缝补型]+粗归因四选一+可选耗时),与 CONTEXT「推进受挫谱系」节对齐 → 核验:canonical 审查通过后 grep 哲学「三字段」命中
- [ ] **哲学 L85 MC-01 补数据源指向**(canonical 修订候选):「可能反例包括返工由需求漂移、AI 能力边界或集成意外主导;若反例连续出现」补一句「反例识别数据源 = 返工事件粗归因字段(CONTEXT 推进受挫谱系节)」→ 核验:同上
- [ ] **哲学三层模型症状层谱系注记**(canonical 修订候选):§一 或方法论 §1.1 三层模型处加注「症状层锚定概念 = 返工,系顶层概念『推进受挫』(含搁置/放弃/返工三结局)的可测量子类」→ 核验:同上
...
- 🔶 **harness 文档治理历史分离**(复压补充声明①,用户裁决「1+2」)→ **design-Q feature(governance-history-split)设计套完成(2026-08-20,L0+L1 两层全链 + 收尾面板)**:范围三域(skill 体系 + harness/design/ 层文件 + CLAUDE.md 仓库状态节,TODO 历史块排除);核心裁决 = 注记全迁(推翻 2026-08-19 缓行评估)/ 双侧常态性形态分工(全局=分发洁净开箱:SKILL.md+引擎+FORK-NOTES,无 DESIGN/CHANGELOG;项目=车间完整)/ 载体三件(skill CHANGELOG 仅项目侧 + FORK-NOTES 双侧一致 + harness/STATUS-LOG 内部状态史)/ sync-check 历史层类规则 + EXCEPTIONS 清空 / 增量协议五类触发 + ADR-0023 升格衔接。设计套 [L0](harness/design/governance-history-split/L0-vision-scope-acceptance.md) + [L1](harness/design/governance-history-split/L1-contract-gov-history-split.md) + [ADR-0024](harness/adr/0024-governance-history-split-dual-form.md) + OD-10/24/8 联动注已落;问卷 4 份归档 [governance-history-split/](harness/questionnaires/archive/governance-history-split/)。**下一步(收尾面板裁决)**:① ~~grill-Q 压测设计套~~ **✅ 已完成(2026-08-20,[grill-gov-history-split-w01](harness/questionnaires/archive/governance-history-split/grill-gov-history-split-w01.md) 单波收口:10 题 9 采推荐 + Q10 推翻,0 逃生舱/0 跑偏;两处已核实硬矛盾 = 全局侧非 git 删除语义 + 外部实证条目归宿冲突;10 项修订全授权已执行——L0 验收 5 改「迁移自身事件即首验」+ L1 九处(删除前置检查 ⚠/DOGFOOD-LOG 双载体 ⚠/引擎头部拆分/锚点更新义务/「规则本体」定义/索引行头部/根 CHANGELOG 义务/TODO 头部压缩/FORK-NOTES 条目级精简)+ ADR-0024 补 Q2 特例与压测回灌注)** → ② long-running 单线程串行执行:P0 协议落盘(HARNESS-RULES 第九节 + sync-check 类规则 + **DOGFOOD-LOG 白名单登记**)→ P1 skill 域 → P2 design 域 → P3 CLAUDE.md 域(+ TODO 头部压缩顺手项)→ P4 全局侧重整(**走删除前置检查:每删前 diff,不一致先抢救**)+ 三脚本验收 + 根 CHANGELOG 记一条(迁移即 dogfood 载体,验收 5 = 本 feature 自身事件入 CHANGELOG 抽查)→ 核验:L0 验收六条 + migration-map_v1 对照
