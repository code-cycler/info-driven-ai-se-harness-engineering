...
**preview 解析(每阶段 W00)**:勾选 `[x]` = 采纳默认 → 按 AI 默认倾向落盘,处理报告逐条标注「来源 = W00 预答」;留空 `[ ]` = 不采纳 → 转入 W01 作为正式题逐个深究(当波 W01 不足时进下一波);W00 无 🤔(yes/no 二选一,定不了即留空);W00 全部勾选(全采纳)合法,处理报告仍列预答清单;W00 预答与 W01 正式题答案冲突时,以 W01 为准并在处理报告记录冲突。
