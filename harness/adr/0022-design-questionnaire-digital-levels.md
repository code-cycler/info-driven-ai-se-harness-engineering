...
- **LN 实测反馈(2026-08-18,grill-Q first-principles W01 ✍️;证据 = 2d rts 项目)**:实测「默认三层 + 同一层同时负责多个任务」导致信息被省略、可能丢失。层分配原则已回灌 STAGE-SKELETONS 总则——**默认每层负责一个功能,L0 恒 vision,仅当某部分内容明显较少时才合并**。
